...
The CYPHO-150 is a relatively inexpensive device, retailing for about \$200.
...
We will be using an AIS decoder produced in the open source GPSD library. Given a data stream sent by our AIS receiver, we need to check that the coordinates of boats that we decode match the coordinates provided by the included AMEC decoder software. The CYPHO-150 can be connected to the AMEC software using the included USB cable.
